...
int main(int argc, char **argv)
{
    ...
    if (argc <= 1) {
        printf("Provide a receipient msgQ name : </msgq-name>\n");
        return 0;
    }
    ...
    if ((msgq_fd = mq_open(argv[1], O_RDONLY|O_CREAT, QUEUE_PEMISSIONS, &attr)) ==
           -1) {
        perror ("mq_open()");
        exit (EXIT_FAILURE);
    } 
